...
void check(int s,map<int,int>m,int arr[])
{
    if(s==1 || m[1]==0 || m[1]==s){cout<<"YES"<<endl; return;}
    else
    {
        if(m[0]>0 && m[1]>0 || m[1]>0 && m[2]>0){cout<<"NO"<<endl; return;}
        else
        {
            for(int i = 0 ; i < s ; i++)
            {
                if(m[arr[i]-1]){cout<<"NO"<<endl; return;}
            }
        }
