...
int d,m,arr[S4],memo[S4][S2][2];
string x,y;
int add(int n1,int n2)
{
    return ((n1%MOD)+(n2%MOD))%MOD;
}
int dp(int i,int n,int sum,int f)
{
    if(i>n)return !(sum);
    int lmt=9,ans=0;
    int& ret = memo[i][sum][f];
    if(~ret)return ret;
    if(!f)lmt=arr[i];
    for(int j = 0 ;j<=lmt ; j++)
    {
        bool baba=f or j<lmt;
        ans= add((dp(i+1,n,((sum+j)%d),baba)),ans);
    }
    return ret=ans%MOD;
}
...
signed main ()
{
    ...
    cin>>x>>d;
    int b;
    for(b = 0 ; b<x.size() ; b++)arr[b+1]=x[b]-'0';
    cout<<1LL*(dp(1,x.size(),0,0)-1+MOD)%MOD;
}